  ...
  */
  bulkInsertQuery: function(tableName, attrValueHashes, options, rawAttributes) {
    ...
    attrValueHashes.forEach(function(attrValueHash) {
      _.forOwn(attrValueHash, function(value, key) {
        if (allAttributes.indexOf(key) === -1) {
          ...
          if(rawAttributes[key].field)
            allAttributesFields.push(rawAttributes[key].field);
          else
            allAttributesFields.push(key);
        }
    ...
    var replacements = {
      ...
      table: this.quoteTable(tableName),
      attributes: allAttributesFields.map(function(attr) {
