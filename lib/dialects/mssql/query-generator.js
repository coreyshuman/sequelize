...
var QueryGenerator = {
  ...
  upsertQuery(tableName, insertValues, updateValues, where, rawAttributes, options) {
    ...
    const attrValueHashUpdate = Utils.removeNullValuesFromHash(updateValues, options.omitNull, options);
    const attrValueHashInsert = Utils.removeNullValuesFromHash(insertValues, options.omitNull, options);
    const primaryKeys = [];
    const updateClauseValues = [];
    const insertClauseValues = [];
    const insertTuples = [];
    const modelAttributeMap = {};

    let query = 'MERGE <%= tableName %> AS [Target] USING (SELECT <%= selectTuples %>) AS [Source] ON (<%= conditionTuples %>) WHEN MATCHED THEN UPDATE SET <%= updateClauseValues %> WHEN NOT MATCHED THEN INSERT (<%= insertTuples %>) VALUES (<%= insertClauseValues %>);';

    if (rawAttributes) {
      Utils._.each(rawAttributes, (attribute, key) => {
    ...
    // determine primary keys for select clause
    for (const attr in modelAttributeMap) {
          if (modelAttributeMap.hasOwnProperty(attr)) {
            let dataType = modelAttributeMap[attr];
            if (dataType.primaryKey === true && insertValues[attr] !== undefined) {
                primaryKeys.push(attr);
            }
        }
    }
    ...
    // build select clause
    const selectTuples = primaryKeys.map(attr =>
        this.escape(insertValues[attr], (modelAttributeMap && modelAttributeMap[attr] || undefined), {context: 'SELECT'}) + ' AS ' + this.quoteIdentifier(attr)
    ).join(',');

    // build condition tuples
    const conditionTuples = primaryKeys.map(attr =>
        '[Target].' + this.quoteIdentifier(attr) + ' = ' + '[Source].' + this.quoteIdentifier(attr)
    ).join(' AND ');

    // build update clause
    for (const key in attrValueHashUpdate) {
      ...
      const value = attrValueHashUpdate[key];
      updateClauseValues.push(this.quoteIdentifier(key) + '=' + this.escape(value, (modelAttributeMap && modelAttributeMap[key] || undefined), { context: 'UPDATE' }));
    }

    // build insert clause
    for (const key in attrValueHashInsert) {
      if (modelAttributeMap && modelAttributeMap[key] &&
          modelAttributeMap[key].autoIncrement === true &&
          !this._dialect.supports.autoIncrement.update) {
        // not allowed to update identity column
      ...
      const value = attrValueHashInsert[key];
      insertTuples.push(this.quoteIdentifier(key));
      insertClauseValues.push(this.escape(value, (modelAttributeMap && modelAttributeMap[key] || undefined), { context: 'INSERT' }));
    }

    const replacements = {
        tableName: this.quoteTable(tableName),
        selectTuples: selectTuples,
        conditionTuples: conditionTuples,
        updateClauseValues: updateClauseValues,
        insertTuples: insertTuples,
        insertClauseValues: insertClauseValues
    };
